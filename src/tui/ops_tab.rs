...
impl OpsState {
    pub fn init(&mut self) {
        self.log
            .push("$ mosaicmem demo  # then manipulate memory".into());
        self.log.push("Building memory store...".into());
        let result = super::runner::run_ops();
        self.log.push(format!(
            "Original store: {} patches",
            result.original_patches
        ));
        self.log.push(format!(
            "manipulation::flip_vertical -> {} patches",
            result.flip_patches
        ));
        self.log.push(format!(
            "manipulation::erase_region(origin, r=2) -> {} patches",
            result.erase_patches
        ));
        self.log.push(format!(
            "manipulation::translate(+10x) -> {} patches",
            result.translate_patches
        ));
        self.log.push(format!(
            "manipulation::splice_horizontal -> {} patches",
...
const OPS: [(&str, &str, &str); 5] = [
    (
        "Original",
        "Store built from 32-frame circular trajectory",
        "memory::store",
    ),
    (
        "Flip Vertical",
        "Mirror all patch Y coordinates across origin",
        "manipulation::flip_vertical",
    ),
    (
        "Erase Region",
        "Remove patches within sphere(origin, r=2)",
        "manipulation::erase_region",
    ),
    (
        "Translate +10x",
        "Shift all patches by (10, 0, 0) in world space",
        "manipulation::translate",
    ),
    (
        "Splice Horizontal",
        "Merge original + translated with 10u offset",
        "manipulation::splice_horizontal",
    ),
];
...
pub fn render(f: &mut Frame, state: &mut OpsState, area: Rect, tick: u64) {
    ...
    f.render_widget(
        Paragraph::new(Line::from(vec![
            ...
            Span::styled(
                " → manipulation::flip_vertical, erase_region, translate, splice_horizontal",
